...
interface LocalUploadQueueState {
  ...
  updateItemPath: (oldPath: string, newPath: string) => void
...
export const useLocalUploadQueue = create<LocalUploadQueueState>()(
  devtools(
    set => ({
      ...
      updateItemPath: (oldPath, newPath) =>
        set(
          state => ({
            // Preserve the item id across rename so id-keyed state (status/progress
            // updates, external subscribers) is not orphaned.
            items: state.items.map(item =>
