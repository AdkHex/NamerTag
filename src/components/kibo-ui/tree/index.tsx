...
export const TreeNodeTrigger = ({
  ...
  return (
    <motion.div
      className={cn(
        'group relative mx-1 flex cursor-pointer items-center rounded-md px-3 py-1.5 transition-all duration-200',
        'hover:bg-accent/50',
  ...
  return (
    <div className="pointer-events-none absolute top-0 bottom-0 left-0">
      ...
      {Array.from({ length: level }, (_, index) => {
        ...
        return (
          <div
            className="absolute top-0 bottom-0 border-border/90 border-l border-dashed"
            key={index.toString()}
            style={{
      ...
      <div
        className="absolute top-1/2 border-border/90 border-t border-dashed"
        style={{
      ...
      {isLast && (
        <div
          className="absolute top-0 border-border/90 border-l border-dashed"
          style={{
            left: (level - 1) * (indent ?? 0) + 12,
            height: '50%',
          }}
...
export interface TreeNodeContentProps extends ComponentProps<
  typeof motion.div
> {
  hasChildren?: boolean
}
...
export interface TreeExpanderProps extends ComponentProps<typeof motion.div> {
  hasChildren?: boolean
}
...
export interface TreeIconProps extends ComponentProps<typeof motion.div> {
  icon?: ReactNode
  hasChildren?: boolean
}
